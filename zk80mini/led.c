...
#define readKeyRow(x) \
	if (PORTAbits.RA0) {g_keymatrix[0]|=(1<<x);} else {g_keymatrix[0]&=(0xFF ^ (1<<x));}\
	if (PORTAbits.RA1) {g_keymatrix[1]|=(1<<x);} else {g_keymatrix[1]&=(0xFF ^ (1<<x));}\
...
void timer1Int(){
	...
	switch (g_TMR1h&(TIMR1HSTEP*15)) {
		...
		// For bright LED, output 0 to RB8-15 and output 1 to RB4-7
		case TIMR1HSTEP*0:
			TRISB=0x7FFF;
			LATB =0x00F0;
			CNPUBCLR=0x00F0;
		...
		// For bright LED, output 1 to RB8-15 and output 0 to RB4-7
		case TIMR1HSTEP*8:
			readKeyRow(0);
			TRISB=0x7FFF;
			LATB =0xFF00;
